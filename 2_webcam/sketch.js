...
function drawDetection(){
  ...
  // if(detections.length >0){
    ...
    if(object.label=="person"){
      ...
      if(object.x < widthC/4+widthC/4 && object.x > widthC/4-widthC/4){
        ...
        totalSecond++;
        let minutes = Math.floor(totalSecond/3600);
        let seconds = Math.floor((totalSecond - hour *3600)/60);
        let milliseconds = totalSecond - (hour*3600 + minute*60);
  ...
  // client.publish('acelab/humanpresent', "hello", { qos: 0, retain: false })
